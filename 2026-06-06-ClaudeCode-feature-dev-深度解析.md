...
description: "逐层拆解 /feature-dev 的七阶段流水线——从代码库勘探到架构设计到质量审查，三个专用 subagent 的完整源码分析和设计哲学。"
---
...
Claude Code 的 `/feature-dev` 把功能开发拆成七个阶段，每阶段有专门的 subagent 执行、有人工决策卡点，确保代码在写之前就已经想清楚了。本文逐层拆解源码和设计哲学。
...
- **不是 `/simplify`** — simplify 是写完后清理代码，feature-dev 是管理从零到一的完整开发流程
...
做的事很直接：问清楚问题、目标、约束，整理成一段总结让你确认。这是防止后面返工的第一道防线。
...
从 `agents/code-explorer.md` 源码可以看到配置：
...
**工具集里没有写操作（没有 Write、Edit）。** 勘探阶段不应该有任何副作用。用 Sonnet 而非 Opus，因为勘探任务更偏向广度搜索，不需要深度推理。
...
即使你说"你定吧"，它也会给出建议然后等你确认。这是刻意设计的安全阀——不让 agent 替你做决定。
...
工具集与 code-explorer 完全相同——仍然是只读。

系统提示有一条反直觉的指令：

> "Make decisive choices - pick one approach and commit."

这里有一个有趣的张力：command 层面让主 agent 启动多个 architect agent 各出一个方案，而每个 architect agent 自身被要求给出一个果断方案。agent 给出一个确定的方案（带理由和 trade-off），主流程把 2-3 个 agent 的不同方案汇总对比，然后让用户选。

**架构是选择题，不是填空题。** 这不是因为 AI 不自信，而是因为架构决策的权重不在技术上，在上下文里——你比 AI 更了解团队偏好、时间压力、长期规划。
...
到 Phase 4 为止，**一行代码都还没写。** 架构设计全部在只读 agent 中完成。
...
实现由主 Agent 直接执行。主 Agent 拥有完整的上下文（Phase 2 勘探结果 + Phase 4 选定的架构蓝图），以及完整工具权限。
...
三个 agent 的工具集和模型完全相同，区别只在 system prompt 和 color。
...
code-reviewer 不会像传统 linter 那样报一大堆 noise。它只报真正值得关注的问题，每个带 `file:line` 引用和具体修复建议。输出按严重度分组：Critical vs Important。
...
没有一个有 `Write` 或 `Edit` 权限。feature-dev 的设计原则很清晰：**subagent 只做分析和建议，不做修改。** 写代码的权力只在 Phase 5 的主 Agent 手中，而且必须用户批准。

| 角色 | 做什么 | 权限 | 模型 | 颜色标识 |
|------|--------|------|------|----------|
| code-explorer | 勘探代码库 | 只读 | sonnet | yellow |
| code-architect | 设计架构 | 只读 | sonnet | green |
| code-reviewer | 审查代码 | 只读 | sonnet | red |
| 主 Agent | 写代码（Phase 5） | 读写 | 当前模型 | — |
